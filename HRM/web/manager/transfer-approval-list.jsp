...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .btn-confirm { display: inline-flex; align-items: center; gap: 6px; background: var(--ok); color: #fff; border: none; padding: 7px 14px; border-radius: 8px; font-weight: 700; font-size: .78rem; text-decoration: none; transition: all .2s; }
    .btn-confirm:hover { background: #059669; color: #fff; transform: translateY(-1px); }
    ...
    .mode-chip.dept-head { background: var(--warn-l); color: #92400e; }
    .mode-chip.hr-mgr    { background: rgba(79,70,229,0.1); color: var(--indigo); }
...
<div class="page-wrapper">
    ...
    <div class="page-main">
        ...
        <!-- WORKFLOW BANNER (2-STEP VISUALIZATION) -->
        <div class="workflow-banner">
            <div class="workflow-step">
                <div class="step-num done"><i class="fas fa-check"></i></div>
                <div class="step-label done">HR Staff tạo đơn</div>
            </div>
            <span class="step-arrow"><i class="fas fa-chevron-right"></i></span>
            <div class="workflow-step">
                <div class="step-num ${viewMode eq 'DEPT_HEAD_APPROVE' ? 'active' : 'done'}">1</div>
                <div class="step-label ${viewMode eq 'DEPT_HEAD_APPROVE' ? 'active' : 'done'}">Trưởng phòng duyệt</div>
            </div>
            <span class="step-arrow"><i class="fas fa-chevron-right"></i></span>
            <div class="workflow-step">
                <div class="step-num ${viewMode eq 'HR_CONFIRM' ? 'active' : 'waiting'}">2</div>
                <div class="step-label ${viewMode eq 'HR_CONFIRM' ? 'active' : 'waiting'}">HR Manager xác nhận</div>
            </div>
            <span class="step-arrow"><i class="fas fa-chevron-right"></i></span>
            <div class="workflow-step">
                <div class="step-num waiting"><i class="fas fa-user-check"></i></div>
                <div class="step-label waiting">Hoàn tất điều chuyển</div>
            </div>
        </div>
            ...
            <c:choose>
                <c:when test="${viewMode eq 'DEPT_HEAD_APPROVE'}">
                    <div class="mode-chip dept-head">
                        <i class="fas fa-user-tie"></i> Bạn đang xử lý: Bước 1 — Phê duyệt Trưởng phòng (Đơn CHỜ DUYỆT)
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="mode-chip hr-mgr">
                        <i class="fas fa-user-shield"></i> Bạn đang xử lý: Bước 2 — Xác nhận cuối HR Manager (Đơn ĐÃ QUA TRƯỞNG PHÒNG)
                    </div>
                </c:otherwise>
            </c:choose>
                            ...
                            <c:when test="${empty approvals}">
                                <tr>
                                    <td colspan="8" class="text-empty">
                                        ...
                                        <c:choose>
                                            <c:when test="${viewMode eq 'DEPT_HEAD_APPROVE'}">Không có yêu cầu điều chuyển nào đang chờ bạn phê duyệt.</c:when>
                                            <c:otherwise>Không có yêu cầu nào đang chờ xác nhận cuối.</c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
                            </c:when>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${tr.status eq 'PENDING'}">
                                                    <span class="badge-s b-pending"><i class="far fa-clock"></i> Chờ TP</span>
                                                </c:when>
                                                ...
                                                <c:when test="${tr.status eq 'APPROVED'}">
                                                    <span class="badge-s b-approved"><i class="fas fa-check"></i> Hoàn tất</span>
                                                </c:when>
                                                <c:otherwise>
