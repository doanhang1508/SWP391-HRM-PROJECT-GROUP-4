    ...
    <%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        <%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
            ...
            <style>
                ...
                .leave-card-header.bg-leave {
                    background: linear-gradient(135deg, #2b6cb0, #4299e1);
                }

                .leave-card-header.bg-ot {
                            ...
                            <div class="leave-card">
                                ...
                                <div class="leave-card-body">
                                    ...
                                    <form action="${pageContext.request.contextPath}/employee/leave" method="POST"
                                        class="leave-form" enctype="multipart/form-data">
                                                ...
                                                <c:forEach var="lType" items="${leaveTypes}">
                                                    <c:choose>
                                                        <c:when test="${lType.paidLeave == 1}">
                                                            <option value="${lType.leaveTypeId}">${lType.typeName} (Có
                                                                lương)</option>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <option value="${lType.leaveTypeId}">${lType.typeName}
                                                                (Không lương)</option>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:forEach>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="number" step="0.5" class="form-control" id="totalDays"
                                                name="totalDays" required placeholder="Ví dụ: 1.5">
                                            <small class="text-muted fst-italic">
                                                * Hệ thống sẽ tự động đối chiếu và loại bỏ các ngày bạn không có lịch phân ca thực tế.
                                            </small>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <label for="attachment" class="form-label"><i
                                                    class="fas fa-paperclip me-1"></i>Tài liệu đính kèm (DOC,
                                                PDF)</label>
                                            <input type="file" class="form-control" id="attachment" name="attachment"
                                                accept=".doc,.docx,.pdf">
                                            <small class="text-muted mt-1 d-block">Có thể đính kèm giấy khám bệnh hoặc
                                                tài liệu liên quan (tối đa 10MB).</small>
                                        </div>
                            ...
                            <ul class="nav nav-tabs" id="historyTabs" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" id="leave-tab" data-bs-toggle="tab"
                                        data-bs-target="#leaveHistory" type="button" role="tab">
                                        ...
                                        <table class="table table-hover align-middle mb-0">
                                            ...
                                            <tbody>
                                                <c:forEach var="lr" items="${leaveHistory}">
                                                    <tr>
                                                        <td>
                                                            <fmt:formatDate value="${lr.createdAt}"
                                                        ...
                                                        <td>
                                                            <c:choose>
                                                                <c:when test="${not empty lr.attachment}">
                                                                    <a href="${pageContext.request.contextPath}/${lr.attachment}"
                                                                        target="_blank" class="text-primary"
                                                                        style="text-decoration: none;"><i
                                                                            class="fas fa-file-download me-1"></i>Xem</a>
                                                                </c:when>
                                                                <c:otherwise>
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${lr.status == 'Approved'}">
                                                                    <span class="badge bg-success"
                                                                        style="border-radius:6px;padding:5px 10px;">Đã duyệt</span>
                                                                </c:when>
                                                                <c:when test="${lr.status == 'Rejected'}">
                                                                    <span class="badge bg-danger"
                                                                        style="border-radius:6px;padding:5px 10px;">Từ chối</span>
                                                                    <c:if test="${not empty lr.rejectReason}">
                                                                        <div class="mt-1" style="font-size:0.8rem; color:#dc3545; font-style:italic;">
                                                                            <i class="fas fa-info-circle"></i> ${lr.rejectReason}
                                                                        </div>
                                                                    </c:if>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <span class="badge bg-warning text-dark"
                                                                        style="border-radius:6px;padding:5px 10px;">Chờ duyệt</span>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </td>
                                                    </tr>
                                                ...
                                                <c:if test="${empty leaveHistory}">
                                                    <tr>
                                                        <td colspan="8" class="text-center text-muted py-4"><i
                                                                class="fas fa-inbox me-2"></i>Chưa có đơn nghỉ phép nào.
                                                        </td>
                                                    </tr>
                                                </c:if>
                                            </tbody>
