...
<c:set var="pageTitle" value="Quản lý Trạng thái Làm việc" scope="request" />
...
<style>
    :root { --navy:#0a2540; --blue:#2b6cb0; --bg:#f0ede8; --surface:#fff; --border:#e2e8f0; --text:#0f172a; --muted:#64748b; }
    * { box-sizing: border-box; }
    ...
    .s-blue{background:#eff6ff;color:#2b6cb0;} .s-green{background:#f0fdf4;color:#16a34a;} .s-purple{background:#faf5ff;color:#7c3aed;}
    ...
    .search-box input { padding: 8px 14px 8px 36px; border: 1px solid var(--border); border-radius: 8px; font-size: .85rem; width: 220px; outline: none; transition: border .2s; }
    ...
    .modal-box { background: var(--surface); margin: 8% auto; padding: 28px 32px; width: 460px; border-radius: 14px; box-shadow: 0 20px 40px rgba(0,0,0,.15); }
    ...
    .btn-cancel { background: none; border: 1px solid var(--border); padding: 9px 18px; border-radius: 8px; font-size: .875rem; cursor: pointer; color: var(--muted); }
...
<div class="page-wrapper">
    <jsp:include page="../shared/sidebar.jsp">
        <jsp:param name="activeMenu" value="employment-status" />
    </jsp:include>
    ...
    <div class="page-main">
        <div class="page-topbar">
            <div class="page-topbar-left">
                ...
                <h1><i class="fas fa-briefcase" style="color:var(--blue);margin-right:10px;font-size:1.3rem;"></i>Quản Lý Trạng Thái Làm Việc</h1>
            </div>
            <button class="btn-primary" onclick="openAddModal()">
                <i class="fas fa-plus"></i> Thêm trạng thái
            </button>
        </div>

        <div class="summary-grid">
            <div class="summary-card">
                <div class="s-icon s-blue"><i class="fas fa-briefcase"></i></div>
                <div>
                    <div class="s-label">Tổng trạng thái</div>
                    <div class="s-value">${fn:length(employmentStatusList)}</div>
            ...
            <div class="summary-card">
                ...
                <div>
                    ...
                    <div class="s-sub">Đã phân loại trạng thái</div>
                </div>
            </div>
            <div class="summary-card">
                <div class="s-icon s-purple"><i class="fas fa-star"></i></div>
                <div>
                    <div class="s-label">Trạng thái phổ biến</div>
                    <div class="s-value" style="font-size:1.1rem;" id="topStatus">—</div>
                    <div class="s-sub" id="topStatusCount"></div>
                </div>
            </div>
        </div>
        ...
        <div class="panel">
            <div class="panel-header">
                <h3 class="panel-title"><span class="dot"></span> Danh Sách Trạng Thái Làm Việc</h3>
                <div class="search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="searchInput" placeholder="Tìm trạng thái..." oninput="filterTable()">
                </div>
            </div>
            <div style="overflow-x:auto;">
                <table class="data-table" id="mainTable">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty employmentStatusList}">
                                <tr><td colspan="6">
                                    <div class="empty-state">
                                        <i class="fas fa-briefcase"></i>
                                        <p style="font-weight:600;color:var(--navy);">Chưa có trạng thái làm việc nào</p>
                                        <p style="font-size:.85rem;">Nhấn "Thêm trạng thái" để bắt đầu</p>
                                    </div>
                                </td></tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach items="${employmentStatusList}" var="es" varStatus="st">
                                    <tr>
                                        <td style="color:var(--muted);font-weight:600;">${st.count < 10 ? '0' : ''}${st.count}</td>
                                        <td>
                                            <div class="item-name">
                                                <div class="item-icon ic-${(st.index % 5) + 1}"><i class="fas fa-briefcase"></i></div>
                                                ${es.statusName}
                                            </div>
                                        </td>
                                        <td><span class="item-desc">${empty es.description ? '—' : es.description}</span></td>
                                        <td style="text-align:center;">
                                            <span class="badge-count" data-emp-count="${empCountMap[es.statusId]}" data-id="${es.statusId}" data-name="${es.statusName}">
...
<div class="modal-overlay" id="addModal">
    <div class="modal-box">
        <div class="modal-header">
            <h3 class="modal-title"><i class="fas fa-plus-circle" style="color:var(--blue);margin-right:8px;"></i>Thêm Trạng Thái Mới</h3>
            <button class="modal-close" onclick="closeModal('addModal')">&times;</button>
        </div>
        <form action="${pageContext.request.contextPath}/admin/employment-status" method="POST">
            <input type="hidden" name="action" value="add">
            <div class="form-group">
                <label class="form-label">Tên trạng thái <span style="color:#e11d48;">*</span></label>
                <input type="text" name="name" class="form-control" placeholder="VD: Đang làm việc" required>
            </div>
            <div class="form-group">
...
<div class="modal-overlay" id="editModal">
    <div class="modal-box">
        <div class="modal-header">
            <h3 class="modal-title"><i class="fas fa-edit" style="color:var(--blue);margin-right:8px;"></i>Cập Nhật Trạng Thái</h3>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('topStatus').textContent = top.name;
        document.getElementById('topStatusCount').textContent = top.count > 0 ? top.count + ' nhân viên' : '';
    });
</script>
