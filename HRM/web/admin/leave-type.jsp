...
// Lý do: đảm bảo bảng đã render trước khi query
    document.addEventListener('DOMContentLoaded', function () {
        ...
        allRows = Array.from(document.querySelectorAll('#leaveTable tbody tr'));

        // Chạy lọc lần đầu để hiển thị đúng trang 1
        filterTable();
    });

// Lọc bảng theo ô tìm kiếm và dropdown paid/unpaid
    function filterTable() {
        // Lấy giá trị tìm kiếm, chuyển về chữ thường để so khớp không phân biệt hoa thường
        const query = document.getElementById('searchInput').value.toLowerCase();

        // Lấy giá trị filter: 'all', 'paid', hoặc 'unpaid'
        ...
        filteredRows = allRows.filter(function (row) {
            ...
            // Tìm badge trong dòng để xác định trạng thái paid/unpaid
            const paidBadge = row.querySelector('.badge-paid, .badge-unpaid');

            // Mặc định là 'paid', ghi đè nếu badge có class 'badge-unpaid'
            let rowPaid = 'paid';
            if (paidBadge && paidBadge.classList.contains('badge-unpaid'))
                rowPaid = 'unpaid';

            // Khớp nếu filter là 'all' hoặc trùng với trạng thái dòng
            const matchPaid = paidVal === 'all' || rowPaid === paidVal;

            // Dòng chỉ hiển thị khi thỏa cả hai điều kiện
...
// Cập nhật hiển thị bảng và các nút phân trang
    function updatePagination() {
        ...
        for (let i = 1; i <= totalPages; i++) {
            // Trang hiện tại tô màu xanh, các trang khác màu trắng
