                                ...
                                <c:forEach items="${departmentList}" var="dept" varStatus="st">
                                    <tr>
                                        ...
                                        <td style="text-align:center;">
                                            ...
                                            <a href="${pageContext.request.contextPath}/admin/department?action=delete&id=${dept.departmentId}"
                                               class="action-btn btn-delete" title="Xóa"
                                               onclick="return confirm('Xóa phòng ban \'${dept.departmentName}\'?')">
...
<script>
    ...
    // Load employee counts via simple fetch (optional: can skip if no API)
    document.addEventListener('DOMContentLoaded', function() {
        const badges = document.querySelectorAll('.badge-count');
        let total = 0;
        let biggest = { name: '—', count: 0 };

        // Read data-emp-count from server if available, else show dash
