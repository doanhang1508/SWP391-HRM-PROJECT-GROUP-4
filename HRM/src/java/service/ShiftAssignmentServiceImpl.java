...
/**
 * ShiftAssignmentServiceImpl — Scheduling coordinator.
 * Builds weekly schedule matrices, batch-assigns shifts,
 * and resolves clock-in/out ambiguity at shift boundaries.
 */
public class ShiftAssignmentServiceImpl implements ShiftAssignmentService {
    ...
    public Map<Integer, Map<Integer, ShiftAssignment>> buildWeeklyScheduleMatrix(LocalDate weekStart) {
        ...
        for (ShiftAssignment sa : assignments) {
            ...
            if (dayIndex < 0 || dayIndex > 6) continue; // Safety bound
    ...
    /**
     * Resolve whether a card swipe is a CHECK_OUT (ending current shift)
     * or a CHECK_IN (starting next shift).
     *
     * Algorithm:
     * 1. Look up the user's assignment for swipeDate.
     * 2. Load the assigned Shift to get start_time and end_time.
     * 3. Calculate how close swipeTime is to end_time vs start_time.
     * 4. If swipeTime is within SWIPE_WINDOW_MINUTES of end_time → CHECK_OUT.
     * 5. If swipeTime is within SWIPE_WINDOW_MINUTES of start_time → CHECK_IN.
     * 6. Also check the NEXT day's assignment (for night shift checkout on next day).
     *
     * Example: Shift1 ends 14:00, Shift2 starts 14:00.
     *   Swipe at 13:55 → |14:00 - 13:55| = 5 min → CHECK_OUT for Shift1.
     *   If user has Shift2 on same day, also 5 min from start → ambiguous.
     *   Resolution: Prefer CHECK_OUT (finishing work) over CHECK_IN (starting work).
     */
    @Override
    public String resolveSwipeIntent(int userId, LocalTime swipeTime, LocalDate swipeDate) {
        ...
        if (todayShift == null) return "UNKNOWN";
    ...
    /**
     * Absolute difference in minutes between two LocalTime values.
     * Handles midnight crossing by taking the minimum of both directions.
     */
